...
def test_verify_raw_body_roundtrip():
    ...
    assert verify_hmac_signature(secret=secret, body=body, signature_header=signature, algo="sha256")
...
def test_reject_modified_body():
    ...
    tampered = b'{"a":2}'
    assert not verify_hmac_signature(secret=secret, body=tampered, signature_header=sig, algo="sha256")
